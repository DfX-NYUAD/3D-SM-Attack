...
This is a simple proximity attack for 3D split manufacturing, as proposed in the following paper: *Patnaik, S.; Ashraf, M.; Sinanoglu, O. & Knechtel, J. Best of Both
...
As of now, we provide the binary, along with sample files.
The latter are for ISCAS85 and ITC99 benchmarks, and we already compiled all the
...
correct connection rates (CCR).  Besides, in reality, such identifiable names would be randomized. As of now, the main heuristics
are to derive a netlist (i) without inducing any combinatorial loops, (ii) without inducing false multi-driver scenarios, and (iii)
while resolving all obfuscated mappings.


To invoke the attack, use something like the following:
